...
void Caja::abrir(){
    ...
    limpiarBuffer();
    cin.getline(monto,10);
    aperturaCaja=atof(monto);
    estadoCaja=true;
    ingresos=0;
    egresos=0;
...
long cant=buscarIdCaja();
    if(cant==0){
        ...
        cout << "\t\tAún no hay cajas cargadas"<< endl;
        pausa();
        return;
    }
Caja aux;
...
if(pos==-1){
    ...
    cout << "\t\tNo se encontró caja con ese id"<< endl;
    pausa();
    return;
...
if(aux.modificarEnDisco(pos)){
    ...
    cout << "\t\tSe sumo monto a caja con éxito"<< endl;
    pausa();
    return;
}
...
void copiarArchivoMemoriaCaja(Caja *v,long cant){
  ...
  void mostrarVectorCaja(Caja *v, long cant){
    ...
    int cont=0;
    for(i=0; i<cant; i++){
        if(v[i].getEstadoCaja()==false){
                cont++;
        v[i].mostrar();
        }
      }
      if(cont==0){
        cout << "\t\tNo hay cajas en el historial"<< endl;
        pausa();
      }
}
