...
bool compararConFechaHoy(int,int,int);
...
bool compararConFechaHoy(int d,int m,int a){
    time_t tiempo;
    struct tm *tmPtr;
    tiempo = time(NULL);
    tmPtr = localtime(&tiempo);
    ///FECHA ACTUAL tmPtr->tm_mday, tmPtr->tm_mon+1, 1900+tmPtr->tm_year
    if(tmPtr->tm_mday==d&&tmPtr->tm_mon+1==m&&1900+tmPtr->tm_year==a)return true;
    return false;
}
...
void Venta::cargar(){
    ...
    Producto aux;
    char pago[3];
    ///int cont=0; para que salga a los 3 errores ///
...
public:
   ...
   /// sets
   void setCosto(float nuevoPrecio){costo=nuevoPrecio;}
   void setTecnologias(const char *nuevasTecno){strcpy(tecnologias,nuevasTecno);}
   void setNombre(const char *nuevoName){strcpy(nombre,nuevoName);}
   void setCamas(const char *nuevaCama){strcpy(camas,nuevaCama);}
   void setVista(const char *nuevoVista){strcpy(vista,nuevoVista);}
...
int diasEstadia=cd(entra,sale);
Tipo_habitacion aux;
int pos=tipoHab-'0';
/// se le resta una unidad para que concuerde con la pos del archivo
aux.leerDeDisco(pos-1);
float precioEstadia=aux.getCosto();
estadiaCompleta=(float)diasEstadia*precioEstadia;
cout << "\t\tValor de la estadia por dia: $"<< precioEstadia << endl;
cout << "\t\tValor de la estadia completa: $"<< estadiaCompleta << endl;
...
cuenta=totalFacturado(numeroDeEstadia)+estadiaCompleta-pago_adelantado;
/*
int diasEstadia=cd(entra,sale);
float estadiaCompleta=(float)diasEstadia*precioEstadia;*/

cout << "\t\tPrecio por noche           --->$"<< precioEstadia << endl;
cout << "\t\tPrecio por estadia completa--->$"<< estadiaCompleta << endl;
...
while(aux.leerDeDisco(pos++)==1){
    ...
    cout << "\t\t";cout <<pos<<")"<< aux.getNombre(); cout<< endl;
}
...
if((aux.getTipo()==_tipo)&&(aux.getMatenimiento()==false)){
...
while(aux.leerDeDisco(pos)==1){
  ...
  if(cant==0){
    cout << "\t\tEn este momento no hay habitaciones en mantenimiento"<< endl;
    pausa();
    return;
  }
...
public:
    bool enviar_mantenimiento(); ///Carga una habitacion al sistema de mantenimiento
    void mostrar();
    ///GETS
    const char* getNumero(){return numero; }
    bool getEstado(){return estado;}
...
bool Habitacion_mantenimiento::enviar_mantenimiento(){

    /// cargar ///
    Habitacion aux;


    /// inicio carga numero de habitacion ///
    cout << "\t\tNumero de habitación que ingresa a mantenimiento: ";
    limpiarBuffer();
    cin.getline(numero,4);
    ...
    int hab=buscarHabitacion(numero);
    if(hab==-1){
        cout << "\t\tNúmero de habitación inexistente"<< endl;
        pausa();
        return false;
        }

    if(verifMantenimiento(numero)==false){
      borrarPantalla();
    ...
    /// fin carga numero de habitacion ///
    ...
    /// aca cambia el estado de la habitacion a true mantenimiento ///
...
while(aux.leerDeDisco(pos++)==1){
    ...
    if ((strcmp(aux.getNumero(),num)==0)&&(aux.getMatenimiento()==true))
        return false;
...
Habitacion reg;
cout << "\t\tNúmero de habitación a habilitar: ";
limpiarBuffer();
cin.getline(hab,4);
while(cadenaVacia(hab)==true){
    borrarPantalla();
    mensajes(3);
    borrarPantalla();
    cout << "\t\tNúmero de habitación a habilitar: ";
...
int posi=buscarHabitacion(hab); /// busca en el archivo hab
int pos=buscarHabMantenimiento(hab);/// busca en el archivo hab_mante
if(posi==-1){

    cout << "\t\tHabitación "<< hab << " no se encontró"<< endl;
    pausa();
    return;
...
if(pos==-1){
    ...
    cout << "\t\tSe habilitó nuevamente habitación "<< hab << endl;
    pausa();
}
...
int posicion=buscarHabitacion(hab); /// busca la habitacion ingresada en archivo habitaciones///
if(posicion==-1){

    cout << "\t\tHabitación " << hab << " es inexistente"<< endl;
    pausa();
    return;
...
int pos=buscarHabMantenimiento(hab); /// buscar en las habitaciones mantenimiento activas ///
if(pos==-1){
...
aux.modificarEnDisco(pos);
cout << "\t\tEl nuevo gasto se sumo al total"<< endl;
...
cout << "\t\tNueva descripción: ";
    ...
    while(cadenaVacia(nuevaDescripcion)){
        borrarPantalla();
        mensajes(3);
        borrarPantalla();
        cout << "\t\tNueva descripción: ";
    ...
    if(aux.modificarEnDisco(pos)){
        cout << "\t\tSe modificó descripción con éxito"<< endl;
        pausa();
    }
...
if (cant==0){
 ...
 while(true){
     ...
     cout << "\t\t3) Ver habitaciones en mantenimiento         " << endl; /// muestra todas juntas o una por numero ///
     cout << "\t\t4) Modificar habitacion en mantenimiento     " << endl; /// modifica lo que se lleva gastado, se agrega una descripcion mas
     cout << "\t\t0) Salir "<< endl;
     cout << "\t\t  Opción: ";
     cin >> opcion;
     borrarPantalla();
...
aux.mostrar();
int dia,mes,anio;
dia=aux.getDiaEntrada();
mes=aux.getMesEntrada();
anio=aux.getAnioEntrada();
if(compararConFechaHoy(dia,mes,anio)==false){

        cout << "\t\tLa fecha de ingreso de la reserva no coinside con el día actual" << endl;
        cout << "\t\tNo se puede llevar a cabo la recepción"<< endl;
        pausa();
        return -1;
